...
현재 저장소는 AWS 퍼블릭 클라우드 베이스라인과 팀 협업용 문서/폴더 구조를 먼저 세팅한 상태입니다. 인프라 1 담당자는 `infra/terraform/aws/network`를 기준으로 네트워크 작업을 이어가면 됩니다.
...
2. `infra/terraform/aws/network/terraform.tfvars.example`를 복사해 실제 값으로 수정합니다.
3. `infra/terraform/aws/network`에서 Terraform plan으로 네트워크 베이스라인을 검증합니다.
...
- `infra/terraform/aws/network`는 현재 VPC, 서브넷, 라우팅, NAT, S3 Endpoint, 보안그룹, ALB 베이스라인까지 포함합니다.
